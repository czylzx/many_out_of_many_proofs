/***********************************************************************************
this hpp implements Bulletproof for Anonymous Zether  
***********************************************************************************/
#ifndef BULLET_PROOF_Z_HPP_
#define BULLET_PROOF_Z_HPP_
...
{
    size_t RANGE_LEN;
    siez_t LOG_RANGE_LEN;
    size_t MAX_AGG_NUM; // number of sub-argument (for now, we require m to be the power of 2) in anonymous zether ,m=2
    ECPoint g, h;
...
std::ifstream &operator>>(std::ifstream &fin, PP& pp)
{
    ...
    pp.vec_g.resize(pp.RANGE_LEN * pp.MAX_AGG_NUM); //reesize the vector
...
std::ofstream &operator<<(std:ofstream &fin,const Proof &proof)
{
...
{ 
    ...
    size_t LEN = pp.RANGE_LEN * n; // LEN = mn,in this case, m=2
